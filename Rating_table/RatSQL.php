...
function getRatingFromDiff($diff){
    $query = "SELECT * FROM aquigaza_fsutt.rating WHERE dif_from <= ".escapeString($diff)." AND dif_to >= ".escapeString($diff);
...
//Gets rating able
function getRatings(){
    $query = "SELECT * FROM aquigaza_fsutt.rating 
...
function addRating($difFrom, $difTo, $expected, $upset){
    $query = "INSERT INTO aquigaza_fsutt.rating 
...
function deleteRating($idRat){
    $query = "DELETE FROM aquigaza_fsutt.rating WHERE id_rating =".escapeString($idRat);
    $resource = runQuery($query);
}

function updateRatingsTable($id, $difFrom, $difTo, $expected, $upset){
    $query = "UPDATE aquigaza_fsutt.rating SET dif_from='".escapeString($difFrom)."', 
