...
function getRatingFromDiff($diff){
    $query = "SELECT * FROM $database.rating WHERE dif_from <= ".escapeString($diff)." AND dif_to >= ".escapeString($diff);
...
//Gets rating able
function getRatings(){
    $query = "SELECT * FROM $database.rating 
...
function addRating($difFrom, $difTo, $expected, $upset){
    $query = "INSERT INTO $database.rating 
...
function deleteRating($idRat){
    $query = "DELETE FROM $database.rating WHERE id_rating =".escapeString($idRat);
    $resource = runQuery($query);
}

function updateRatingsTable($id, $difFrom, $difTo, $expected, $upset){
    $query = "UPDATE $database.rating SET dif_from='".escapeString($difFrom)."', 
